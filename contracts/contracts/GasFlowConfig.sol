...
contract GasFlowConfig is Ownable2Step, Pausable {
    ...
    function _validateFeeAgainstOracle(
        ...
        uint256 ethUsd = _readPrice(ethUsdFeed);
        uint256 tokenUsd = _readPrice(tokenUsdFeed);
        ...
        uint256 expectedFee = (ethAmount * ethUsd * (10 ** tokenDec)) / (1e18 * tokenUsd);
     ...
     *         2. Record the fee in the stake pool (fee already transferred by Delegator)
     *         3. Compensate the relayer with ETH from the stake pool
     *
     *         SECURITY MODEL:
     ...
     *         - Stablecoin transfer happens BEFORE this call (in Delegator.execute via transfer)
     */
    function processCompensation(
        ...
        // ── Record + compensate (fee already transferred to stakePool by Delegator) ──
    ...
    function setStalenessThreshold(uint256 _stalenessThreshold) external onlyOwner {
        require(_stalenessThreshold >= 1 minutes, "GasFlowConfig: threshold too low");
